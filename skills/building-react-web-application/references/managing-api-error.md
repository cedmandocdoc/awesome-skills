...
Use this guide to keep **user-facing error copy** in `src/api/` and keep TanStack Query and UI layers thin. Every failed API call throws the same error type; hooks pass it through; routes and components only decide **where** and **when** to show `error.message`.
...
| ----- | ---- | ------------- |
| **`src/api/`** | Catching transport/service failures, mapping to `ApiError`, choosing **user-facing `message`** (reuse backend copy when valid; otherwise a safe fallback). | React, routing, toasts vs inline layout. |
| **Feature hooks** (`useQuery` / `useMutation`) | Calling API functions; letting failures propagate. | Rewriting error messages for display. |
| **Routes / components** | Passing `query.error` into error UI; showing `error.message` on initial failure; retry via `refetch`. | Parsing Axios, reading `response.data`, or duplicating fallback strings. |
...
2. **Map only when needed** — unknown status, missing payload, network failure, or non-user-facing technical text. Substitute a short, generic fallback (for example “Something went wrong. Please try again.”).
...
Adjust `ErrorBody` in `utils.ts` to match the backend contract. When the transport changes (for example Supabase → REST), update **`utils.ts` and the rare inline maps** in `modules/`—not feature hooks or routes.
...
| Concern | Rule |
| ------- | ---- |
| **Initial load error** | Treat `query.isError` as failed first fetch; display `query.error.message` in route or feature error UI. |
| **Background refetch** | Do not replace the whole view with a new message; keep showing cached data. Optional toast is a **placement** choice, not a new copy source. |
...
Use the fallback in `QueryError` only as a last resort for unexpected non-API failures; the API layer should prevent those in query paths.
...
- [ ] UI uses `error.message` (with `isApiError` when narrowing), not Axios types or `response.data`.
