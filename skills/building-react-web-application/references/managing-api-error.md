...
2. Map only when needed—missing payload, network failure, or non-user-facing text. Use a short generic fallback (for example “Something went wrong. Please try again.”).
...
- **Initial load error:** show `query.error.message` in route or feature error UI.
- **Background refetch:** keep cached data visible; optional toast is placement only, not a new copy source.
