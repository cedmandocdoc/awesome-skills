...
Configure TypeScript to match React Navigation's static type system:
...
- `strict` (or at minimum `strictNullChecks`) is required for route param inference and IntelliSense.
...
import { WorkshopDetail } from "@/features/workshop-detail";

type WorkshopDetailProps = StaticScreenProps<{
  workshopId: string;
}>;

function WorkshopDetailScreen({ route }: WorkshopDetailProps) {
  return <WorkshopDetail workshopId={route.params.workshopId} />;
}
...
- Type each screen's `route.params` with `StaticScreenProps<...>`.
