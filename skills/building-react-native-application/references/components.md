...
### Guidelines for abstracting components

This section explains how to decide whether a component should be created as a shared UI primitive (in `src/ui/`) or as a feature component (in `src/features/<feature-name>/components/`), and when to extract it at all.

#### When building UI components

- Use registry components first before creating a custom one.
  - Check `src/ui/` for an existing shared primitive.
  - If it is missing, add it from the registry (via the Expo project root script used elsewhere in this doc).
...
#### Mental model for component categorization and extraction
...
3. Extract only if it stays reasonably small.
   - Verdict for extraction: aim for components under 200 lines (not restricted, but recommended).
