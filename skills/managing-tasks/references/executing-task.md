...
Runtime truth: `<task-folder>/status.md` then `plan.md`. Infra: [task-contract.md](./task-contract.md) → **Resolve tasks root**, **Finding existing tasks**, **Resolving domain references**, **Step queue rules**, **Auto-archive**, **`index.md` status mirror**.
...
3. If `overall_status` is `Cancelled` → stop; recreate via [creating-task.md](./creating-task.md) if needed
...
2. Apply **Requirements** — open Sources URLs/paths the step depends on; honor Constraints and Acceptance
3. Load Context **Skills to load**; read each skill's `SKILL.md`
...
2. If pending implementation steps remain → stop; finish them first or replan ([updating-task.md](./updating-task.md))
3. Keep `overall_status`: `In Progress`; `next_step_id`: `verify`
4. Execute every item in `plan.md` → **Verification checklist** (automated checks and listed smoke steps only — no new features)
5. Record pass/fail per item

**If all checks pass** → **§5 Mark Done and archive**.
...
3. Set `overall_status`: `Done`; `current_step_id`: `verify`; `next_step_id`: `none`
4. Update `handoff_note`: task complete
5. Append session log: date, `verify`, `Completed`, brief results
6. Auto-archive per [task-contract.md](./task-contract.md) → **Auto-archive**

### 6. Update `status.md` after a non-verify step

| After step | Update |
| --- | --- |
| Step completed | Check off step; set todo `status: completed` in `plan.md` frontmatter |
| More steps remain | Set `current_step_id` to completed step; `next_step_id` to following step |
| Only `verify` remains | Keep `overall_status`: `In Progress`; `next_step_id`: `verify` |

Always update `handoff_note`, append **Session log** (date, step id, result, brief notes), and sync `<tasks-root>/index.md` when `overall_status` changed.

### 7. Blocked workflow

If a blocker prevents finishing the step, follow [blocking-task.md](./blocking-task.md) (or apply the same updates inline): set `Blocked`, set `blocking_reason`, leave `next_step_id` and checkmarks unchanged, log the attempt.

If the user only wants to unblock without implementing, use [blocking-task.md](./blocking-task.md) → **Unblock**.

### 8. Confirm to the user

Reply with step completed, verification results, or blocked reason; new `next_step_id` and `overall_status` (or archived path when `Done`); suggested follow-up if work remains.

## Examples

**Execute:** Continue `tasks/001-dark-mode-toggle` → read `status.md` first → run `next_step_id`.

**Verify and archive:** `next_step_id` is `verify` → checklist → on pass move to `archives/`.
